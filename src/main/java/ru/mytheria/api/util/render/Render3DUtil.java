...
import ru.mytheria.api.util.color.ColorUtil; // Ваш ColorUtil
...
public final class Render3DUtil {
    ...
    private static final Identifier PARTICLE_TEXTURE = Identifier.of("textures/bloom.png"); // Используем заглушку
    ...
    /* ==========================
       CALL FROM WORLD RENDER (ВАШ КОД)
       ========================== */
    ...
    private static void quad(BufferBuilder b, Texture t) {
        ...
        // Порядок вершин (против часовой стрелки, если смотреть сверху)
        // В Minecraft рендерере 3D текстуры часто требуют специфичного порядка.
        // Я оставляю ваш исходный порядок:
    ...
    public static void updateAndDrawWorldParticles(MatrixStack matrices, float tickDelta, int maxParticles, float particleSpeed, float particleSize, int baseColor) {
        ...
        // ВАЖНО: MathUtil.interpolate() и ProjectionUtil не предоставлены,
        // используем mc.player.getPos() в качестве заглушки
